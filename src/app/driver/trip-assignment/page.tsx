...
export function DriverTripAssignments({
  user,
  viewMode,
}: {
  user: UserRole;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1>
            {viewMode === "active"
              ? "Active Trips"
              : `Trip Assignments for ${user.name}`}
          </h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {viewMode === "active" ? "Active Trips" : "Today's Assignments"}
          </CardTitle>
          <CardDescription>
            {filteredTrips.length} trips{" "}
            {viewMode === "active" ? "in progress" : "assigned for today"}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
      ...
      <Dialog open={isActionModalOpen} onOpenChange={setIsActionModalOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {actionType === "start" ? "Start Trip" : "End Trip"} -{" "}
              {selectedTrip?.id}
            </DialogTitle>
            <DialogDescription>
              {actionType === "start"
            ...
            <div className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div>
                    <strong>Route:</strong> {selectedTrip.fromLocation} →{" "}
                    {selectedTrip.destination}
                  </div>
                  <div>
                    <strong>Scheduled:</strong> {selectedTrip.scheduledTime} -{" "}
